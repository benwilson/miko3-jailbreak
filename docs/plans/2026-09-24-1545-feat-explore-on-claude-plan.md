...
- KTD5. **Faces are cropped with Android's built-in FaceDetector inside the chosen person box.** The hit is expanded 1.6x and saved as a 224x224 JPEG. No new model is added. (Amended 2026-09-25 after the owner found a stored face showing the wall: the face is cut from a fresh look taken once he has turned to the person and stopped, inside the detector's person box that IoU-matches the pick (else Claude's box in its own scan frame), with FaceDetector run on an RGB_565 copy. The top-25% stand-in is gone: with no face found, nothing is matched or stored, the person is asked their name as someone new, and the line after their reply makes no promise to remember them.) Implements R9, R13.

...
- KTD6. **Retrace drives forward, not backward.** Backing up is blind. The retrace walks the leg log backwards: for each leg, newest first, it turns to face the reverse of that leg and drives its distance forward, stopping once the total reaches the retrace distance, so the floor sensor and camera guard every step. A hazard during the retrace ends it and moves on to the scan.
...
- **Approach:**
  ...
  2. Retrace (KTD6) uses the leg log.

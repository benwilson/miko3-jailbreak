...
- KTD6. **Retrace drives forward, not backward.** Backing up is blind. The retrace walks the leg log backwards: for each leg, newest first, it turns to face the reverse of that leg and drives its distance forward, stopping once the total reaches the retrace distance, so the floor sensor and camera guard every step. A hazard during the retrace ends it and moves on to the scan. When turns are blocked, he first reverses straight along the most recent forward leg, capped at its logged distance — ground he just drove over — then turns. (Added after live QA 2026-09-25: wedged under a desk, measured turns physically could not rotate.) When no logged leg qualifies, any blocked measured turn (roaming, the escape's turns, the first move after a rest) first backs up blind a few ticks (`blockedTurnBackTicks`, stopped early by a stall, not logged as a leg) and retries that turn once; a second block proceeds as before. (Added after live QA 2026-09-25: pinned after a CPL stop, all he needed was to back up a little and then spin.) The retried turn goes the other way (to the same target the other way round, or the same amount for a turn whose way doesn't matter), and a way found blocked is avoided by every later turn until he drives off cleanly, even the long way round; the back-up is about 2 s (`blockedTurnBackTicks` 8). (Added after live QA 2026-09-25: left was blocked while right and reversing were free, and every retry went left again.) Once wedged, by any trigger, the ladder's first move is that same blind back-up (`blockedTurnBackTicks`), whatever the leg log holds, once per ladder and with no leg back-out after it; if it moved him, the turn that would not turn is tried the free way and he drives off (wedged otherwise, a forward try), and a retrace turn the long way round past a blocked side over `retraceLongWayMaxDeg` (200°) is skipped for the circle. (Added after live QA 2026-09-25: nose to a wall, the turn into it blocked, the retrace spent 20 s on a 327° turn when "he could just back up to get out of there".)
...
- **Approach:**
  ...
  2. Retrace (KTD6) uses the leg log. When turns are blocked, he first reverses straight along the most recent forward leg, capped at its logged distance — ground he just drove over — then turns. A measured turn the gyro says is not turning (under ~5° of progress in ~1.5 s) is blocked: it stops at once instead of waiting out the 15 s backstop, and outside an escape it counts as wedged. The wheel counters are signed and cumulative (live 2026-09-25: reverse counts down, from 0 at power-up), so `SensorReply` reads a leading minus and wheel presence is its own flag on `SensorSnapshot` and `SensorReading`; before that, every negative count read as absent and back-ups, stalls and legs went blind. Wedged, he first backs up blind about 2 s, then (if that moved him) turns the free way and drives off before the retrace; a retrace turn over 200° the long way past a blocked side is skipped for the circle. The learned turn rate that sizes the step budgets learns only from turns that reached their amount (20° or more), never a blocked or cut-short one (live 2026-09-25: those dragged it to the 15°/s floor while real turns ran ~40°/s).

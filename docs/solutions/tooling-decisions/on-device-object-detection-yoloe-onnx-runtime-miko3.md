...
- Measured on the MT8168 at 640x480: session create ~750 ms, first frame 1.3-1.5 s, steady ~1.1 s on **2** intra-op threads. **4 threads is slower** (1.6-1.9 s): it competes with the brain, UI and camera. The same model runs in 25 ms on an Apple-silicon Mac and gave the same detections as the robot, so the Mac is a good place to check vocabulary changes (`--check IMAGE`).

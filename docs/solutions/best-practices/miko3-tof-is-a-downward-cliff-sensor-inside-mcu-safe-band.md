...
- Standing on carpet at about 175 sits just above the MCU's band floor (about 170). The MCU's own refusal (`CPL=2`) therefore still fired about 3 times in 6 minutes of roaming. It is firmware; app thresholds cannot remove it, and it must not be overridden (point 3). Since then (owner-approved, 2026-09-25) a refusal while his own sensor reads plain floor gets one retry of the leg after a 400 ms pause, and another refusal within 1.5 s is a hazard as always (`ExploreTuning.cplRetryPauseMs`, `cplRetryWindowMs`). That asks again; it never overrides the refusal.

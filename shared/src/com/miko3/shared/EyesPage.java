...
/**
 * The robot's on-screen eyes, shared by every mode that shows them (U7, KTD9):
 * the remote-control mode's idle screen (DeviceViewPage) and the voice mode's
 * device view. Each mode composes its own page from these parts, adding its own
 * CSS, markup, and script around the eyes; the eyes themselves live only here.
 *
 * The remote-control composition is pinned byte-for-byte to the page that mode
 * served before U7 by scripts/tests/test_eyes_page_golden.py. Any edit to the
 * strings below changes the remote-control mode's screen too, and that test
 * fails until its golden is recaptured on purpose.
 *
 ...
 * eyes looking together rather than two independent ones. No dedicated face
 * asset exists in this project, so the eyes are drawn with plain CSS/JS
 * rather than shipping a new image.
 *
 ...
 *
 * State hook for callers: the eyes sit in a #rig element whose descendants are
...
public final class EyesPage {
    ...
    private static final String LENS_CSS =
    ...
    private static final String RIG = "<div id=\"rig\">" + LENS + LENS + "</div>";
    ...
    /**
     * Builds a full eyes page. Every argument is inserted verbatim (HTML/CSS/JS,
     * not text to escape); pass "" for an empty slot.
     *
     * @param title             the page's &lt;title&gt; text
     * @param extraCss          appended after the eyes' own rules, so it can override them
     * @param extraMarkup       placed right after the #rig element
     * @param scriptBeforeEyes  runs before the eyes start (the page's poll, typically)
     * @param scriptAfterEyes   runs after gazeTo/nextGlance are defined and the
     *                          first glance is scheduled (e.g. to wrap gazeTo)
     */
    public static String build(String title, String extraCss, String extraMarkup,
                               String scriptBeforeEyes, String scriptAfterEyes) {
        return "<!doctype html><html><head><meta charset=\"utf-8\">"
                + "<meta name=\"viewport\" content=\"width=device-width, initial-scale=1\">"
                + "<title>" + title + "</title>"
                + "<style>" + LENS_CSS + extraCss + "</style>"
                + "</head><body>"
                + RIG
                + extraMarkup
                + "<script>" + scriptBeforeEyes + EYES_JS + scriptAfterEyes + "</script>"
                + "</body></html>";
    }
}

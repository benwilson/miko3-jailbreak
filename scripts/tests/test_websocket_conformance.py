...
def javac_cmd(javac, out_dir, sources):
    ...
    sourcepath = os.pathsep.join([str(SHARED_SRC), str(HARNESS / "stubs")])
    return [javac, "-source", "8", "-target", "8", "-encoding", "UTF-8", "-Xlint:-options",
            "-sourcepath", sourcepath, "-d", str(out_dir)] + [str(s) for s in sources]
